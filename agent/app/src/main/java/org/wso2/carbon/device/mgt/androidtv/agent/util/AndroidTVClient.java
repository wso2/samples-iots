...
import org.wso2.carbon.device.mgt.androidtv.agent.util.dto.RegisterInfo;
...
public class AndroidTVClient {
    ...
    public RegisterInfo register(String username, String password, String deviceId, android.os.Handler mUiHandler) {
        ...
        if (responseStatus.trim().contains(TVConstants.Request.REQUEST_SUCCESSFUL)) {
    ...
    public Map<String, String> registerWithTimeWait(String username, String password, String deviceId) {
        try {
            AndroidTVClientAsyncExecutor androidTVClientAsyncExecutor = new AndroidTVClientAsyncExecutor(context);
            String endpoint = LocalRegistry.getServerURL(context);
            androidTVClientAsyncExecutor.execute(username, password, deviceId, endpoint);
            Map<String, String> response = androidTVClientAsyncExecutor.get();
            if (response != null) {
                return response;
            }
