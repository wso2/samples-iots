...
var summerHtmlImageMapCreator = (function() {
	...
	var app = (function() {
		...
		function onSvgClick(e) {
			if (isAddDeviceMode) {
				var deviceCoords = utils.getRightCoords(e.pageX, e.pageY);
				var x = deviceCoords.x;
				var y = deviceCoords.y;
				var content = $("#device-response-template");
				var deviceButton = content.find("#xCord");
				if (deviceButton) {
					deviceButton.remove();
				}
				deviceButton = content.find("#yCord");
				if (deviceButton) {
					deviceButton.remove();
				}

				var cordbutton = '<input type="hidden" class="form-control" id="xCord" name="xCord" value="' + x +'"> ' +
					'<input type="hidden" class="form-control" id="yCord" name="yCord" value="'+y+'">';
				content.find("#device-button").append(cordbutton);
