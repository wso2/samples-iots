 ...
 * under the License.
 */
package org.wso2.carbon.device.mgt.android_tv.mqtt;
...
import org.wso2.carbon.device.mgt.android_tv.handlers.impl.HDMIHandlerImpl;
...
import org.wso2.carbon.device.mgt.android_tv.mqtt.transport.TransportHandlerException;
...
public class AndroidSenseMQTTHandler extends MQTTTransportHandler {
     ...
     * @param ctx is the android context object.
     * @return AndroidSenseMQTTHandler.
     */
    public static AndroidSenseMQTTHandler getInstance(Context ctx) {
        context = ctx;
        if (mInstance == null) {
    ...
    public void processIncomingMessage(MqttMessage mqttMessage, String... messageParams) {
        if (messageParams.length != 0) {
            ...
            AudioHandler volumeHandler = new AudioHandlerImpl(context);
            HDMIHandler hdmiHandler = new HDMIHandlerImpl(context);
            String msg;
            msg = mqttMessage.toString();
            Log.d(TAG, "MQTT: Received Message [" + msg + "] topic: [" + topic + "]");
            if (topic.contains("volume")) {
                if (msg.toLowerCase().contains("off")) {
                    volumeHandler.mute();
                } else if (msg.toLowerCase().contains("on")){
            ...
            } else if (topic.contains("hdmi")) {
                if (msg.toLowerCase().contains("on")) {
                    hdmiHandler.enable();
                } else if (msg.toLowerCase().contains("off")) {
                    hdmiHandler.disable();
                } else {
                    String errorMsg =
                            "MQTT message [" + mqttMessage.toString() + "] was received without the topic information.";
