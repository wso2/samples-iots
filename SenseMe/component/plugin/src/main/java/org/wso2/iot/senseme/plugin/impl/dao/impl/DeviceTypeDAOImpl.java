...
package org.wso2.iot.senseme.plugin.impl.dao.impl;
...
public class DeviceTypeDAOImpl {
    ...
    public Device getDevice(String deviceId) throws DeviceMgtPluginException {
        ...
        try {
            ...
            String selectDBQuery =
                    "SELECT senseme_DEVICE_ID, DEVICE_NAME" +
                            " FROM senseme_DEVICE WHERE senseme_DEVICE_ID = ?";
            ...
            if (resultSet.next()) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("senseme device " + deviceId + " data has been fetched from " +
                            "senseme database.");
                }
            }
        } catch (SQLException e) {
            String msg = "Error occurred while fetching senseme device : '" + deviceId + "'";
            log.error(msg, e);
    ...
    public boolean addDevice(Device device) throws DeviceMgtPluginException {
        ...
        try {
            ...
            String createDBQuery =
                    "INSERT INTO senseme_DEVICE(senseme_DEVICE_ID, DEVICE_NAME) VALUES (?, ?)";
            ...
            if (rows > 0) {
                status = true;
                if (log.isDebugEnabled()) {
                    log.debug("senseme device " + device.getDeviceIdentifier() + " data has been" +
                            " added to the senseme database.");
                }
            }
        } catch (SQLException e) {
            String msg = "Error occurred while adding the senseme device '" +
                    device.getDeviceIdentifier() + "' to the senseme db.";
            log.error(msg, e);
            throw new DeviceMgtPluginException(msg, e);
        } finally {
    ...
    public boolean updateDevice(Device device) throws DeviceMgtPluginException {
        ...
        try {
            ...
            String updateDBQuery =
                    "UPDATE senseme_DEVICE SET  DEVICE_NAME = ? WHERE senseme_DEVICE_ID = ?";
            ...
            String msg = "Error occurred while modifying the senseme device '" +
    ...
    public boolean deleteDevice(String deviceId) throws DeviceMgtPluginException {
        ...
        try {
            ...
            String deleteDBQuery =
                    "DELETE FROM senseme_DEVICE WHERE senseme_DEVICE_ID = ?";
            ...
            if (rows > 0) {
                status = true;
                if (log.isDebugEnabled()) {
                    log.debug("senseme device " + deviceId + " data has deleted" +
                            " from the senseme database.");
                }
            }
        } catch (SQLException e) {
            String msg = "Error occurred while deleting senseme device " + deviceId;
            log.error(msg, e);
            throw new DeviceMgtPluginException(msg, e);
        } finally {
            DeviceTypeUtils.cleanupResources(stmt, null);
    ...
    public List<Device> getAllDevices() throws DeviceMgtPluginException {
        ...
        try {
            ...
            if (log.isDebugEnabled()) {
                log.debug("All senseme device details have fetched from senseme database.");
            }
            return iotDevices;
        } catch (SQLException e) {
            String msg = "Error occurred while fetching all senseme device data'";
            log.error(msg, e);
            throw new DeviceMgtPluginException(msg, e);
        } finally {
            DeviceTypeUtils.cleanupResources(stmt, resultSet);
