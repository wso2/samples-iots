 ...
 * under the License.
 */
package org.wso2.carbon.device.mgt.androidtv.agent.mqtt;
...
import org.wso2.carbon.device.mgt.androidtv.agent.MessageActivity;
import org.wso2.carbon.device.mgt.androidtv.agent.VideoActivity;
...
import org.wso2.carbon.device.mgt.androidtv.agent.mqtt.transport.TransportHandlerException;
...
public class AndroidTVMQTTHandler extends MQTTTransportHandler {
    private static final String TAG = "AndroidTVMQTTHandler";
    private static volatile AndroidTVMQTTHandler mInstance;
     ...
     */
    public static AndroidTVMQTTHandler getInstance(Context ctx) {
        context = ctx;
        if (mInstance == null) {
            Class clazz = AndroidTVMQTTHandler.class;
            synchronized (clazz) {
                if (mInstance == null) {
                    mInstance = new AndroidTVMQTTHandler(context);
                }
            }
        }
        return mInstance;
     ...
     */
    private AndroidTVMQTTHandler(Context context) {
        super(context);
    }
    ...
    public void processIncomingMessage(MqttMessage mqttMessage, String... messageParams) {
        if (messageParams.length != 0) {
            ...
            if (topic.contains("video")) {
                ...
                intent.putExtra(TVConstants.MESSAGE, msg);
