...
package org.homeautomation.androidtv.plugin.impl.dao.impl;
...
import org.homeautomation.androidtv.plugin.impl.util.DeviceTypeUtils;
...
/**
 * Implements IotDeviceDAO for androidtv Devices.
 */
public class DeviceTypeDAOImpl {
        ...
        try {
            ...
            if (resultSet.next()) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("androidtv device " + deviceId + " data has been fetched from " +
                            "androidtv database.");
                }
            }
        } catch (SQLException e) {
            String msg = "Error occurred while fetching androidtv device : '" + deviceId + "'";
            log.error(msg, e);
    ...
    public boolean addDevice(Device device) throws DeviceMgtPluginException {
        ...
        try {
            ...
            if (rows > 0) {
                status = true;
                if (log.isDebugEnabled()) {
                    log.debug("androidtv device " + device.getDeviceIdentifier() + " data has been" +
                            " added to the androidtv database.");
                }
            }
        } catch (SQLException e) {
            String msg = "Error occurred while adding the androidtv device '" +
                    device.getDeviceIdentifier() + "' to the androidtv db.";
            log.error(msg, e);
            throw new DeviceMgtPluginException(msg, e);
        } finally {
    ...
    public boolean updateDevice(Device device) throws DeviceMgtPluginException {
        ...
        try {
            ...
            String msg = "Error occurred while modifying the androidtv device '" +
    ...
    public boolean deleteDevice(String deviceId) throws DeviceMgtPluginException {
        ...
        try {
            ...
            if (rows > 0) {
                status = true;
                if (log.isDebugEnabled()) {
                    log.debug("androidtv device " + deviceId + " data has deleted" +
                            " from the androidtv database.");
                }
            }
        } catch (SQLException e) {
            String msg = "Error occurred while deleting androidtv device " + deviceId;
            log.error(msg, e);
            throw new DeviceMgtPluginException(msg, e);
        } finally {
            DeviceTypeUtils.cleanupResources(stmt, null);
