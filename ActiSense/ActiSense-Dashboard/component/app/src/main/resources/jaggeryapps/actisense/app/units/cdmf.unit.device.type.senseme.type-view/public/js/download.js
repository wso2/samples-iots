...
function downloadAgent() {
    ...
    var invalidUserNameErrMsg = $("#invalid-username-error-msg");
    if (deviceName && deviceName.length < 4) {
        invalidUserNameErrMsg.find("span").text("Device name should be more than 3 letters!");
        invalidUserNameErrMsg.removeClass("hidden");
        ...
        invalidUserNameErrMsg.find("span").text("Invalid device name");
        invalidUserNameErrMsg.removeClass("hidden");
    }
}
...
function doAction(data) {
    ...
    if (data.status == "200") {
        ...
        $("input#download-device-url").val(data.responseText).focus(function () {
            $(this).select();
        });
        showAgentDownloadPopup();
